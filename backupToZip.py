...
def backup_zip(directory):
    ...
    zipFilename = os.path.join(parentDir, os.path.basename(directory)) + formatToday + '.zip'

    # Ensure the folder exists.
    if not os.path.exists(directory):
        print('\nFolder "{}" does not exist!'.format(directory))
        sys.exit(1)

    # Check if there is already a ZIP backup saved for the same date.
    if os.path.exists(directory + formatToday):
        print('\nBackup ZIP of "{}" for {} already exists!'.format(directory, formatToday))
        sys.exit(1)
    ...
    for foldername, subfolders, filenames in os.walk(directory):
        ...
        for filename in filenames:
            newBase = os.path.basename(directory) + '_'
...
def main():
    ...
    backupDir = os.path.abspath(sys.argv[1])    # Ensure the folder is absolute.
    backup_zip(backupDir)
